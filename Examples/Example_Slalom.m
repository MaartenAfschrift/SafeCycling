...
[pairedttest,p,ci,stats1] = ttest2(DataMatrix(iYoung,4),DataMatrix(iOld,4),0.05); %DataMatrix kolom 4 = ROM FrameTorso
[pairedttest,p2,ci,stats2] = ttest2(DataMatrix(iYoung,8),DataMatrix(iOld,8),0.05); %DataMatrix kolom 8 = ROM PelvisTorso
for speed_ID = 1:2
    if speed_ID == 1
        speed_ID_Sel = DataMatrix(:,3) == 1
        [pairedttest,p3,ci,stats3] = ttest(DataMatrix(:,5),speed_ID_Sel,0.05); %DataMatrix kolom 5 = BoolElderly, kolom 3 = Speed ID
        
    else speed_ID_Sel = DataMatrix(:,3) == 2
        [pairedttest,p4,ci,stats4] = ttest(DataMatrix(:,5),speed_ID_Sel,0.05); %DataMatrix kolom 5 = BoolElderly, kolom 3 = Speed ID
    end
end
...
if ComputeDataMatrix
    ...
    header_DataMatrix =  {'s-ID','bike-ID','Speed-ID','ROM','BoolElderly','Error'}; % header for the datamatrix
                            ...
                            if isfield(Phases,'Trunk') && isfield(Phases,'Pelvis') && isfield(Phases,'Frame')
                                ...
                                if ~isempty(Rpelvis)
                                    ...
                                    if ~isempty(ROM)
                                        ...
                                        h = figure();
                                        set(gcf,'Position',[835   505   531   611])
                                        subplot(3,1,1)
                                        plot(tpelvis,eulpelvis(:,1),'b'); hold on;
                                        plot(ttorso,eulTorso(:,1),'r');
                                        plot(tframe,eulframe(:,1),'k');
                                        ...
                                        legend('pelvis','torso','frame');
                                        saveas(gcf,fullfile(figPathRaw,['subject ' num2str(s) ' ' Folders{f} ' ' OrderMeas{i} '.png']),'png');
                                        close(h);
                                    end
                                end
                            end        
